...
class Bottle2neck(nn.Module):
    """ Res2Net/Res2NeXT Bottleneck type C
...
def res2net50_14w_8s(pretrained = False, num_classes = 1000, in_chans = 3, **kwargs):
    ...
    if pretrained:
        load_pretrained_model(model, 'conv1', in_chans = 3, url = model_urls['res2net50_14w_8s'])
    return model
...
def res2net50_26w_4s(pretrained = False, num_classes = 1000, in_chans = 3, **kwargs):
    ...
    if pretrained:
        load_pretrained_model(model, 'conv1', in_chans = 3, url = model_urls['res2net50_26w_4s'])
    return model
...
def res2net50_26w_6s(pretrained = False, num_classes = 1000, in_chans = 3, **kwargs):
    ...
    model = ResNet(Bottle2neck, [3, 4, 6, 3], base_width = 26,
                   num_classes = num_classes, in_chans = in_chans, block_args = res2net_block_args, **kwargs)
    if pretrained:
        load_pretrained_model(model, 'conv1', in_chans = 3, url = model_urls['res2net50_26w_6s'])
    return model
...
def res2net50_26w_8s(pretrained = False, num_classes = 1000, in_chans = 3, **kwargs):
    ...
    model = ResNet(Bottle2neck, [3, 4, 6, 3], base_width = 26,
                   num_classes = num_classes, in_chans = in_chans, block_args = res2net_block_args, **kwargs)
    if pretrained:
        load_pretrained_model(model, 'conv1', in_chans = 3, url = model_urls['res2net50_26w_8s'])
    return model
...
def res2net50_48w_2s(pretrained = False, num_classes = 1000, in_chans = 3, **kwargs):
    ...
    model = ResNet(Bottle2neck, [3, 4, 6, 3], base_width = 48,
                   num_classes = num_classes, in_chans = in_chans, block_args = res2net_block_args, **kwargs)
    if pretrained:
        load_pretrained_model(model, 'conv1', in_chans = 3, url = model_urls['res2net50_48w_2s'])
    return model
...
def res2net101_26w_4s(pretrained = False, num_classes = 1000, in_chans = 3, **kwargs):
    ...
    model = ResNet(Bottle2neck, [3, 4, 23, 3], base_width = 26,
                   num_classes = num_classes, in_chans = in_chans, block_args = res2net_block_args, **kwargs)
    if pretrained:
        load_pretrained_model(model, 'conv1', in_chans = 3, url = model_urls['res2net101_26w_4s'])
    return model
...
def res2next50(pretrained = False, num_classes = 1000, in_chans = 3, **kwargs):
    ...
    model = ResNet(Bottle2neck, [3, 4, 6, 3], base_width = 4, cardinality = 8,
                   num_classes = num_classes, in_chans = in_chans, block_args = res2net_block_args, **kwargs)
    if pretrained:
        load_pretrained_model(model, 'conv1', in_chans = 3, url = model_urls['res2next50'])
    return model


def res2net50_v1b_26w_4s(pretrained = False, num_classes = 1000, in_chans = 3, **kwargs):
    """Constructs a Res2Net-50_v1b_26w_4s model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    res2net_block_args = dict(scale = 4)
    model = ResNet(Bottle2neck, [3, 4, 6, 3], base_width = 26, stem_width = 32, stem_type = 'deep', avg_down = True,
                   num_classes = num_classes, in_chans = in_chans, block_args = res2net_block_args, **kwargs)

    if pretrained:
        load_pretrained_model(model, 'conv1', in_chans = 3, url = model_urls['res2net50_v1b_26w_4s'])
    return model


def res2net101_v1b_26w_4s(pretrained = False, num_classes = 1000, in_chans = 3, **kwargs):
    """Constructs a Res2Net-50_v1b_26w_4s model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    res2net_block_args = dict(scale = 4)
    model = ResNet(Bottle2neck, [3, 4, 23, 3], base_width = 26, stem_width = 32, stem_type = 'deep', avg_down = True,
                   num_classes = num_classes, in_chans = in_chans, block_args = res2net_block_args, **kwargs)

    if pretrained:
        load_pretrained_model(model, 'conv1', in_chans = 3, url = model_urls['res2net101_v1b_26w_4s'])
    return model
