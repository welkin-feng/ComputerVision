"""efficientnet.py - Model and module class for EfficientNet.
...
class EfficientNet(nn.Module):
    ...
    Example:
        >>> import torch
        >>> from cvmodels.models import EfficientNet
...
def load_pretrained_weights(model, model_name, weights_path=None, load_fc=True, advprop=False):
    ...
    Args:
        ...
        advprop (bool): Whether to load pretrained weights
                        trained with advprop (valid when weights_path is None).
    """
    skip = ['_fc.weight', '_fc.bias'] if load_fc else []

    if isinstance(weights_path, str):
        ret = load_pretrained_model(model, model_path=weights_path, skip=skip)
    else:
        # AutoAugment or Advprop (different preprocessing)
        url_map_ = url_map_advprop if advprop else url_map
        ret = load_pretrained_model(model, url=url_map_[model_name], skip=skip)

    assert set(ret.missing_keys) == set(skip), \
...
def load_pretrained_model(model, model_path='', url='', skip=(), conversion=()):
    ...
    Args:
        ...
        model_path: path of pretrained weight
        url: pretrained weight's url, download from this url if model_path is not exist
        skip: skip keys in pretrained model's state_dict
        conversion: tuple/list of conversion pair tuples/lists, like (new_name, original_name)

    Returns:
        None if model_path not exist and url invalid, else model.load_state_dict()'s return
    """
    ...
    print(f"=> loading pretrained model weight length {len(pretrained_state_dict)} / ",
          f"total_state_dict {len(state_dict)} / total_model_dict {len(model_dict)}")
...
def effcientnet_b0(pretrained=False, in_chans=3, **kwargs):
    ...
    if pretrained:
        load_pretrained_model(model, url=url_map['efficientnet-b0'])
    if in_chans != 3:
        model._change_in_channels(in_chans)
    return model
